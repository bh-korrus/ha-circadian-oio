...
8. **No options UI for the curve yet.** Curve exponent, cap values, time zones, transition windows are all hard-coded in `const.py`. Don't expose these as user-tunable until we have a story for what "good" tuning looks like — random users moving the late-night cap to 50% defeats the entire point.
