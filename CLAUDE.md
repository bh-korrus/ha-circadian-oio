...
8. **The schedule, daytime arc endpoints, and bulb floor are user-tunable; the curve is not.** The Options flow exposes eight knobs — night start time, night end time, transition duration, night brightness cap, daytime peak CCT (arc top at solar noon), daytime base CCT (arc shoulders at sunrise/pre-sunset), minimum brightness, and minimum CCT — stored in `entry.options`. The first six are behavior; the last two are per-bulb hardware floors. The incandescent dim curve itself (exponent, `BASE_CCT`, the L\* phase split, the evening CCT cap, the `_arc_shape` square-root) stays hard-coded in `const.py`/`render.py`; those define the circadian behavior and aren't safe to hand to arbitrary users. The options form's description warns that lifting the night cap or cooling the night color weakens the effect. If you add more knobs, keep schedule/intensity tunable and leave the curve math fixed.
